    ...
    const fetchUserData = async () => {
      ...
      try {
        ...
        if (userData) {
          ...
          // Fetch habits
          const habitsRef = collection(db, `users/${userData.id}/habits`);
          ...
          setColors(darkMode ? darkGradientColors[colorKey].colors : lightGradientColors[colorKey].colors);
        }
      } catch (error) {
      ...
      <div className="relative flex flex-wrap lg:flex-nowrap lg:space-x-5">
        <div className="flex-1 flex flex-col space-y-5">
          <div className="flex items-center space-x-4 mb-4">
            <img src={profileUser.avatarURL || '/default-avatar.png'} alt={profileUser.username} className="w-20 h-20 rounded-full" />
            <div>
              <h1 className="text-3xl font-bold text-white">{profileUser.username}</h1>
              <p className="text-gray-300">{profileUser.location}</p>
            </div>
          </div>
