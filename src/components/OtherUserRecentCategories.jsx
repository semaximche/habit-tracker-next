...
const OtherUserRecentCategories = ({ userId }) => {
    ...
    const fetchUserData = async () => {
      try {
        ...
        if (userDoc.exists()) {
          ...
          const habitsCollection = await getDocs(collection(db, `users/${userId}/habits`));
          const habits = {};
  ...
  if (isLoading) {
    return <Container title="Recent Categories" pad={true}><div>Loading...</div></Container>;
  }
  ...
  if (!userData) {
    ...
    const categories = Object.keys(userData.habits).reduce((acc, habitKey) => {
        ...
        if (habit.lastCompleted && (!acc[category].lastCompleted || habit.lastCompleted > acc[category].lastCompleted)) {
